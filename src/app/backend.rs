...
pub struct RenderPass {
    ...
    /// Map of egui texture IDs to textures and their associated bindgroups (texture view + sampler).
    /// The texture may be None if the TextureId is just a handle to a user-provided sampler.
    textures: AHashMap<egui::TextureId, (Option<wgpu::Texture>, wgpu::BindGroup)>,
}
...
impl RenderPass {
    ...
    pub fn new(
        ...
        let shader = wgpu::include_wgsl!("backend.wgsl");
        let module = device.create_shader_module(&shader);

        let uniform_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("egui_uniform_buffer"),
            contents: bytemuck::bytes_of(&UniformBuffer {
                screen_size: [0.0; 4],
            }),
            ...
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                ...
                entries: &[wgpu::BindGroupLayoutEntry {
                    ...
                    ty: wgpu::BindingType::Buffer {
                        ...
                        min_binding_size: NonZeroU64::new(size_of::<UniformBuffer>() as u64),
        ...
        let uniform_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[wgpu::BindGroupEntry {
                ...
                resource: wgpu::BindingResource::Buffer(wgpu::BufferBinding {
                    ...
                    offset: 0,
                    size: NonZeroU64::new(size_of::<UniformBuffer>() as u64),
                }),
            }],
        });
    ...
    pub fn execute<'rpass>(
        ...
        for ((egui::ClippedMesh(mut clip, mesh), vertex_buffer), index_buffer) in paint_jobs
            .iter()
            ...
            .zip(self.index_buffers.iter())
        {
            ...
            clip.min.x = (clip.min.x * screen.scale).clamp(0.0, screen.width as f32);
            clip.min.y = (clip.min.y * screen.scale).clamp(0.0, screen.height as f32);
            clip.max.x = (clip.max.x * screen.scale).clamp(clip.min.x, screen.width as f32);
            clip.max.y = (clip.max.y * screen.scale).clamp(clip.min.y, screen.height as f32);

            clip.min = viewport.clamp(clip.min);
            clip.max = viewport.clamp(clip.max);

            let clip_min_x = clip.min.x.round() as u32;
            let clip_min_y = clip.min.y.round() as u32;
            let clip_max_x = clip.max.x.round() as u32;
            let clip_max_y = clip.max.y.round() as u32;

            {
                // Clip scissor rectangle to target size.
                let x = clip_min_x.min(screen.width);
                let y = clip_min_y.min(screen.height);
                let width = (clip_max_x - clip_min_x).min(screen.width - x);
                let height = (clip_max_y - clip_min_y).min(screen.height - y);
            ...
            let bind_group = self.texture_bind_group(mesh.texture_id)?;
    ...
    pub fn add_textures(
        ...
        for (&texture_id, image_delta) in textures.set.iter() {
            ...
            let origin = image_delta
                ...
                .map_or(wgpu::Origin3d::ZERO, |[x, y]| wgpu::Origin3d { x, y, z: 0 });
            ...
            let data: &[u8] = match &image_delta.image {
                egui::ImageData::Color(c) => bytemuck::cast_slice(c.pixels.as_slice()),
                // The unwrap here should never fail as alpha_srgb_pixels will have been set to `Some` above.
            ...
            match self.textures.entry(texture_id) {
                Entry::Occupied(mut entry) => match image_delta.pos {
                    None => {
                        ...
                        let (texture, _) = entry.insert((Some(texture), bind_group));
                    ...
                    Some(_) => {
                        if let Some(texture) = entry.get().0.as_ref() {
                            ...
                            queue.write_texture(texture_copy, data, data_layout, size);
                        } else {
                ...
                Entry::Vacant(entry) => {
                    ...
                    entry.insert((Some(texture), bind_group));
                }
            }
        }
    ...
    pub fn remove_textures(&mut self, textures: egui::TexturesDelta) -> Result<(), BackendError> {
        for texture_id in textures.free {
            let (texture, _binding) = self.textures.remove(&texture_id).ok_or_else(|| {
                // This can happen due to a bug in egui, or if the user doesn't call `add_textures` when required.
    ...
    pub fn update_egui_texture_from_wgpu_texture_with_sampler_options(
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some(&format!(
                "egui_user_{}_texture_bind_group",
                self.next_user_texture_id
            )),
            layout: &self.texture_bind_group_layout,
            entries: &[
    ...
    pub fn update_buffers(
        ...
        let screen_size = {
            let w = (screen.width as f32 / screen.scale).floor();
            let h = (screen.height as f32 / screen.scale).floor();
            [w, h, w.recip(), h.recip()]
        };
        ...
        self.update_buffer(
            ...
            BufferType::Uniform,
            0,
            bytemuck::bytes_of(&UniformBuffer { screen_size }),
        );
    ...
    fn update_buffer(
        ...
        if data.len() > buffer.size {
            ...
            buffer.buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some(&format!("egui_{}_buffer", name)),
                contents: data,
